...
Образ приложения `kazhem/pymongo_api:1.0.0` уже содержит логику кэширования через
`fastapi-cache` — она включается автоматически, как только в окружении появляется
...
TTL — 60 секунд (`@cache(expire=60)` в коде образа), по истечении кэш протухает
и следующий запрос снова идёт в MongoDB.
